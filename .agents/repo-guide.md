# tw-agent-plugin Repo Guide

These instructions are specific to the tw-agent-plugin repository. They are
loaded only when working in this repo (via `opencode.json` `instructions`),
not globally.
...
This repo is a plugin for **opencode**. When the user says "update the system prompt", "add an instruction", "inject a rule", or similar — they mean adding a persistent instruction that gets injected into every agent session, not editing a one-off config file.

### How it works

Persistent instructions are defined as exported string constants in `src/`, then injected at session start by `src/opencode/index.ts`: the combined rules text is prepended to the first user message in `experimental.chat.messages.transform` (via the `COMBINED_RULES` array).
...
1. Create `src/<name>-rules.ts` (follow the pattern of `src/tool-priority-rules.ts` or `src/git-commit-rules.ts`):
   ...
   ```
2. Import it in `src/opencode/index.ts` and add `MY_RULES` to the `COMBINED_RULES` array.
