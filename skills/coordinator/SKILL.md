...
- **Messaging**: to hand a running agent a task, ask it a question, or relay
  ...
  busy). It frames each message with a sender header, so it cannot deliver a
  slash-command for expansion — use `workmux send` for those (e.g. `/merge`), or
  phrase the request naturally (e.g. "use your /merge skill"). Use the remaining
  `workmux` commands (`status`, `wait`, `capture`, `run`, `add`, `remove`,
  `merge`) for lifecycle and inspection.
...
Because `send-to-agent` prepends a sender header, a slash-command in the message
is not at the start of the input and will NOT be expanded. Use `workmux send` to
type raw input into an agent's TUI pane — chiefly slash-commands the agent must
run itself (e.g. `/merge`, `/commit`), or long input from a file:
