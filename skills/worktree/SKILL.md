...
3. Run `workmux add <worktree-name> -A -b -P <temp-file>` to create the worktree

**ALWAYS pass an explicit `<worktree-name>` AND the `-A` flag.** Without an
explicit name, workmux slugifies the entire prompt into the branch/window name,
producing absurd 20-30 word tmux titles. The explicit name controls the title;
`-A` is a safety net so the auto-namer (capped at 2-4 words) is used if the
explicit name is ever missing. Never run `workmux add` with only `-P`/`-p`.
...
tmux new-window -t <session> -c <project-path> \
  "workmux add <worktree-name> -A -b -P <prompt-file>; exit"
```
...
```bash
workmux add feature-x -A -b -P /tmp/tmp.abc123.md
workmux add feature-y -A -b -P /tmp/tmp.def456.md
```
