...
Prefer purpose-built CLIs over guessing or asking the user to paste content. All are read-only and safe to run during an investigation:

| Source | Tool | Use for |
|--------|------|---------|
| Grafana / telemetry | `gcx` | Alert rules, datasource queries (Prometheus/Loki), dashboards |
| GitHub links | `gh` | Runbooks, related PRs/issues, recent deploys, commit history |
| Slack links | `slackcli` | Resolving Slack permalinks to channel/thread context |
| Kubernetes state | `kubectl` | Live pod/deployment/event state when infra is a suspected cause |

Whenever the user pastes a link, route it to the right tool: GitHub URL → `gh`, Slack URL → `slackcli resolve`, Grafana URL → `gcx`.
...
`gh`, `slackcli`, and `kubectl` are optional but used when relevant links or infra suspicions arise. If one is missing or unauthenticated, note it and continue with the others.
...
- Runbook URLs (if the URL is a GitHub URL and `gh` is available, fetch with `gh api` — see references)
- Dashboard links
- Descriptions

If a deployment is a suspected cause and a GitHub repo is known, check recent merges with `gh`:
```bash
gh pr list --repo <owner>/<repo> --state merged --limit 10 \
  --json number,title,mergedAt,author
gh api /repos/<owner>/<repo>/commits --jq '.[0:10] | .[] | {sha: .sha[0:7], msg: .commit.message, date: .commit.committer.date}'
```

If the alert annotations, labels, or the user reference a Slack discussion, resolve the link with `slackcli` to pull in context (see references):
```bash
slackcli resolve '<slack-url>' --run --json
```

If infrastructure is a suspected cause and you have cluster access, inspect live Kubernetes state for the affected namespace/workload (see references):
```bash
...
- If `gh`, `slackcli`, or `kubectl` is missing or unauthenticated, note it briefly and continue with the tools that work — don't block the investigation
...
For alert JSON structure, query patterns by alert type, graph interpretation, runbook fetching, and the `gh` / `slackcli` / `kubectl` command patterns, see:
